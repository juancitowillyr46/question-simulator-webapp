...
import { PlansService } from '../../maintainers/plans/plans.service';
...
export class ExamsEnabledComponent implements OnInit {
  ...
  // @ViewChildren("contentOne") 
  // public contentOne: ElementRef;
  ...
  ngOnInit() {
    ...
    const that = this;
    that.getPlanObservable.currentMessage.subscribe( res => {
      if(res != null) {
        that.userProfile = res['data'];

        //const dateExpiry = that.userProfile.plan.dateExpiry;

        //const fechaRegistro = new Date(dateExpiry);
        
        // let fechaActual = new Date();
        // if(fechaActual.getTime() > fechaRegistro.getTime()) {
        //   that.planExpired = true;
        // } else {
        //   that.planExpired = false;
        // }
  ...
  private getQuestionsByKeyCategory(category: any) {
    ...
    that.questionsService.getQuestionsByKeyCategory(category.clientKey).subscribe( res => {
      if(res) {
        ...
        localStorage.setItem('seconds', category.durationTimeSec);
        ...
        setTimeout(() => {
          ...
          that.routers.navigateByUrl('/exams/quizzed/'+ category.clientKey +'/questions/1');
        }, 1000);
      }
    });
  }
